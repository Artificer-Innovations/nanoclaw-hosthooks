...
describe("runner hook registries", () => {
  it("isolates observer failures and reports async and slow observers once", () => {
    ...
    registerProviderMessageObserver("async", (() =>
  ...
  it("runs inbound observers with immutable-shaped batch context", () => {
    const seen: unknown[] = [];
    const remove = registerInboundBatchObserver("trace", (context) =>
      seen.push(context)
    );
  ...
  it("shallow-merges query contributors and skips malformed or throwing ones", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    registerProviderQueryOptionsContributor("one", () => ({
      a: 1,
      same: "first",
    }));
  ...
  it("runs provider query-start observers with stage context", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    const seen: unknown[] = [];
    const remove = registerProviderQueryStartObserver("trace", (context) => {
      seen.push(context);
    });
    registerProviderQueryStartObserver("throw", () => {
      throw new Error("boom");
    });
    runProviderQueryStartObservers({
      provider: "claude",
    ...
    expect(seen).toEqual([
      { provider: "claude", stage: "sdk_query", hasContinuation: true },
    ]);
    expect(warn.mock.calls.flat().join(" ")).toContain("throw");
  });
  ...
  it("validates registrations, supports unregister, and reports capabilities", () => {
    expect(() =>
      registerProviderMessageObserver("", () => undefined)
    ).toThrow();
    expect(() => registerProviderMessageObserver("bad", null as never)).toThrow(
      TypeError
    );
    ...
    registerProviderQueryStartObserver("query-start", () => undefined);
    expect(getHosthooksCapabilities()).toMatchObject({
      apiVersion: 1,
      features: { providerQueryStart: true },
      counts: {
  ...
  it("probes capabilities without throwing on loader failure", () => {
    expect(
      probeHosthooksCapabilities(() => getHosthooksCapabilities()).present
    ).toBe(true);
    expect(
      probeHosthooksCapabilities(() => {
