...
/** Lifecycle stages around provider.query / harness boot / session init. */
export type ProviderQueryStartStage =
  | "provider_query"
  | "sdk_query"
  | "session_init";

export type ProviderQueryStartContext = Readonly<{
  provider: string;
  stage: ProviderQueryStartStage;
  hasContinuation?: boolean;
}>;

export type ProviderQueryStartObserver = (
  context: ProviderQueryStartContext
) => void;
...
const providerQueryStartObservers: NamedRegistration<ProviderQueryStartObserver>[] =
  [];

function assertRegistration(name: string, callback: unknown): void {
  if (!name.trim())
    throw new Error("Hosthook registration name must not be empty");
  if (typeof callback !== "function")
    throw new TypeError(`Hosthook "${name}" must be a function`);
}
...
export function registerProviderQueryStartObserver(
  name: string,
  observer: ProviderQueryStartObserver
): () => void {
  return addRegistration(providerQueryStartObservers, name, observer);
}
...
export function runInboundBatchObservers(
  ...
  for (const registration of inboundBatchObservers) {
    runObserver("inbound-batch", registration, () =>
      registration.callback(context)
    );
  }
}

export function runProviderQueryStartObservers(
  context: ProviderQueryStartContext
): void {
  for (const registration of providerQueryStartObservers) {
    runObserver("provider-query-start", registration, () =>
      registration.callback(context)
    );
  }
}
...
export function getHosthooksCapabilities(): {
  ...
  return {
    ...
    counts: {
      ...
      providerQueryStart: providerQueryStartObservers.length,
    },
  };
}
  ...
  try {
    ...
    if (elapsed > OBSERVER_BUDGET_MS) {
      warnOnce(
        ...
        `Observer "${registration.name}" took ${elapsed.toFixed(
          1
        )}ms; advisory budget is ${OBSERVER_BUDGET_MS}ms (not enforced — synchronous JS cannot be preempted).`
      );
    }
  }
}

export type HosthooksCapabilitiesSnapshot = ReturnType<
  typeof getHosthooksCapabilities
>;
...
export function resetHosthooksForTests(): void {
  ...
  providerQueryStartObservers.length = 0;
