...
describe("installer", () => {
  it("installs, verifies, upgrades idempotently, and uninstalls", () => {
    ...
    expect(installed.changed).toHaveLength(11);
    expect(installed.version).toBe("0.2.0");
    expect(
      fs.existsSync(path.join(root, ".claude/skills/add-hosthooks/SKILL.md"))
    ).toBe(true);
    expect(runVerify(root)).toEqual({ root, ok: true, issues: [] });

    const upgraded = runUpgrade(root);
    expect(upgraded.changed).toEqual([]);
    expect(upgraded.unchanged).toHaveLength(11);
    ...
    expect(removed.changed).toHaveLength(7);
    expect(removed.removed).toContain("src/hosthooks.ts");
    expect(removed.removed).toContain("src/warn-once.ts");
    expect(fs.readFileSync(path.join(root, "src/router.ts"), "utf8")).toBe(
      fixtureSources.router
    );
  });
  ...
  it("reports missing files and call sites during verification", () => {
    ...
    expect(result.issues).toContain(
      "src/router.ts missing hosthooks call sites"
    );
  ...
  it("syncs the bundled skill independently", () => {
    ...
    expect(
      fs.readFileSync(path.join(destination, "nested/two.md"), "utf8")
    ).toBe("two");
