...
describe('CLI', () => {
  it('parses commands and tolerates unknown flags', () => {
    expect(parseArgs(['node', 'bin'])).toEqual({ command: 'help', path: undefined });
    expect(parseArgs(['node', 'bin', 'install', '--other', '--path', '/tmp/host'])).toEqual({
      command: 'install',
      path: '/tmp/host',
    });
  });
  ...
  it('turns command exceptions into exit code one', () => {
    vi.spyOn(console, 'error').mockImplementation(() => undefined);
    expect(runCommand(['node', 'bin', 'install', '--path', '/does/not/exist'])).toBe(1);
  });
